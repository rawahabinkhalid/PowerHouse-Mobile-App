...
const EditAllotedMarketScreen = ({ navigation, route }) => {
    ...
    useEffect(() => {
      if(ProjectData.Screen == 'Recurring'){
        setPID(ProjectData.EditData.Id)
      }
      else{
        setPID(ProjectData.EditData.Scout_alloted_id)
      }
        FetchProduct()
        FetchDepartment()
        FetchCompetitor()
        ...
        setOwnerDetail(ProjectData.EditData.OwnerName)
        setOwnerPhoneNumber(ProjectData.EditData.OwnerContactNumber)
        setECity(ProjectData.EditData.City)
        setEArea(ProjectData.EditData.Area)
        setAddress(ProjectData.EditData.Location)
        setStatusType(ProjectData.EditData.Client)
        setStatus(ProjectData.EditData.UserStatus)
        setComment(ProjectData.EditData.Comment)
        if(ProjectData.EditData.ProductCategory == ""){
            setFinalProduct([])
        }
        else{
            setFinalProduct(ProjectData.EditData.ProductCategory.split(","))
        }
        if(ProjectData.EditData.Compitetor == ""){
            setFinalOurAndCompetitorBrand([])
        }
        else{
            setFinalOurAndCompetitorBrand(ProjectData.EditData.Compitetor.split(","))
        }

            navigation.addListener('focus',()=>{
              const backAction = () => {
                if(ProjectData.Screen == 'Recurring'){
                  navigation.navigate("RECURRINGMEETING")
                }
                else{
     ...
     const UpdateMarket = async() =>{
       setLoading(true)
        if(MarketType == null)
        {
          ...
          fetch('https://mobileapp.powerhouse.com.pk/AdminPortal/Api/UpdateAllotedMarketApi.php',{
            method: 'POST',
            headers: {
                "Content-Type": "application/json"
            },
...
export default EditAllotedMarketScreen;