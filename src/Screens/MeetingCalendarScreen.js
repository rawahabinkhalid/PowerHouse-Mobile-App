...
const MeetingCalendarScreen = ({ navigation }) => {
  ...
  //////////////////////////////Market//////////////////
    ...
    const ViewTeam = async (val) => {
     ...
     const GetAllMeetingDates = async() => {
      //setLoading(true)
     try {
         const UserInfo = await AsyncStorage.getItem('UserInfo')
         console.log(UserInfo)
         fetch('https://mobileapp.powerhouse.com.pk/AdminPortal/Api/GetAllMeeting_DateApi.php',{
       method: 'POST',
       headers: {
           "Content-Type": "application/json"
       },
     ...
     .catch((error) => {
      ...
      // Alert.alert("","Please check your internet connection!")
       //setLoading(false)
         ...
         //setLoading(false)
            ...
            const GetMeeting = async(value) => {
              //console.log('===>',value)
              setLoading(true)
            try {
                const UserInfo = await AsyncStorage.getItem('UserInfo')
                console.log(UserInfo)
                fetch('https://mobileapp.powerhouse.com.pk/AdminPortal/Api/GetMeeting_DateApi.php',{
              method: 'POST',
              headers: {
                  "Content-Type": "application/json"
              },
        ...
        try {
            const UserInfo = await AsyncStorage.getItem('UserInfo')
            fetch('https://mobileapp.powerhouse.com.pk/AdminPortal/Api/UpdateSetMeetingApi.php',{
          method: 'POST',
          headers: {
              "Content-Type": "application/json"
          },
        ...
        showsVerticalScrollIndicator= {false}
            >
            {/* <Image
                style={{width: 250, height: 50, marginTop: 40, marginBottom: 30, alignSelf: 'center'}}
            ...
            <FlatList
            scrollEnabled={true}
            style={{marginTop: 15, width: '92%', alignSelf: 'center', marginBottom: 10}}
            data={Meeting}            
            ...
            return(
                ...
                style={{marginLeft: 5, width: '80%'}}>
                    ...
                    <Text style={{color: 'grey',  fontFamily: 'Karla-Bold', paddingRight: 10}}>{item.Location}</Text>
                    <Text style={{color: '#000',  fontFamily: 'Karla-Bold', paddingRight: 10}}>{moment(item.Meeting_Date).format('DD-MM-YYYY')}</Text>
               ...
               // if(moment().format('YYYY-MM-DD')+" "+moment().format('hh:mm A') >= item.Meeting_Date+" "+item.Meeting_Time){
                ...
                navigation.navigate("MEETINGDETAIL",{
                  ...
                  Alert.alert("Meeting Date "+moment(item.Meeting_Date).format('DD-MM-YYYY'),"You cannot start the meeting before the scheduled date.")
                }

                }}
                style={{height: 30, width: '40%', backgroundColor: '#f8ae4e', justifyContent: 'center', alignItems: 'center', borderRadius: 5}}>
                <Text style={{color: '#fff', fontFamily: 'Karla-Bold'}}>Start Meeting</Text>
                </TouchableOpacity>
                ...
                style={{height: 30, width: '40%', backgroundColor: '#f8ae4e', justifyContent: 'center', alignItems: 'center', borderRadius: 5}}>
                <Text style={{color: '#fff', fontFamily: 'Karla-Bold'}}>Edit Meeting</Text>
                </TouchableOpacity>
      ...
      />
      :
      <Text style={{color: '#f8ae4e', alignSelf: 'center', marginTop: 30, fontFamily: 'Karla-Bold', fontSize: 17}}>No Meeting Found</Text>
      }
