...
const AddDepartmentScreen = ({ navigation }) => {
    ...
    const findDepartment = (text) => {
        if (text) {
          const regex = new RegExp(`${text.trim()}`, 'i');
            if(DepartmentList.filter((cityname) => cityname.DepartmentName.search(regex) >= 0).length != 0){
              setDepartmentList(DepartmentList.filter((cityname) => cityname.DepartmentName.search(regex) >= 0))
              setSearch(text)
            }
            else{
              setDepartmentList(AllDepartmentList)
              //setSearch('')
              ToastAndroid.show("No Department found", ToastAndroid.SHORT)
      ...
      const AddDepartment = async() => {
        ...
        setLoading(true)
           fetch('https://mobileapp.powerhouse.com.pk/AdminPortal/Api/DepartmentidApi.php',{
         method: 'POST',
         headers: {
             "Content-Type": "application/json"
         },
       ...
       .then((text) => {
         console.log('==>', text)
    //////////////////////////////////////////
    fetch('https://mobileapp.powerhouse.com.pk/AdminPortal/Api/AddDepartmentApi.php',{
         method: 'POST',
         headers: {
             "Content-Type": "application/json",
         },
       ...
       .then((text) => {
         Alert.alert("","Department Add Successfully")
         setDepartment(null)
         FetchDepartment()
         setLoading(false)
       })
    ...
    const EditDepartmentt = async() => {
      setLoading(true)
      fetch('https://mobileapp.powerhouse.com.pk/AdminPortal/Api/EditDepartmentApi.php',{
         method: 'POST',
         headers: {
             "Content-Type": "application/json"
         },
       ...
       .then((txt) => {
        Alert.alert("","Department Update Successfully")
         setopacity(1)
    ...
    const DeleteDepartment = async(value) => {
      console.log('==>',value)
      setLoading(true)
      fetch('https://mobileapp.powerhouse.com.pk/AdminPortal/Api/DeleteDepartmentApi.php',{
         method: 'POST',
         headers: {
             "Content-Type": "application/json"
         },
       ...
       .then((txt) => {
        console.log(txt)
        Alert.alert("","Department Delete Successfully")
        FetchDepartment()
         setLoading(false)
       })
       .catch((error) => {
    ...
    if(Loading == true){
      ...
      return(
        ...
        <View style={{ elevation: 10, flexDirection: 'row', height: 60, backgroundColor: '#fff', alignItems: 'center', justifyContent: 'space-between', opacity : opacity}}>
            ...
            <View>
            <Text style={{fontFamily: 'Poppins-Bold', color: '#000', fontSize: 20, paddingTop: 7}}>Add Department</Text>
            </View>
        ...
        label="Search"
        value={Search}
        onChangeText={text => findDepartment(text)}
        />
        </View>
        ...
        <View style={{backgroundColor: '#f8ae4e', width: '50%', height: 50, justifyContent: 'center', borderLeftWidth: 1, borderLeftColor: '#fff'}}>
            <Text style={{color: '#fff', fontFamily: 'Karla-Bold', paddingLeft: 10}}>Department Name</Text>
        </View>
        ...
        <FlatList
            data={DepartmentList}
            keyExtractor={(DepartmentList)=>DepartmentList.Id}
        ...
        <View style={{backgroundColor: '#fff', width: '50%', height: 50, justifyContent: 'center', borderLeftWidth: 1, borderLeftColor: '#f8ae4e', elevation: 5, marginBottom: 10}}>
            <Text style={{color: '#000', fontFamily: 'Karla-Bold', paddingLeft: 10}}>{item.DepartmentName}</Text>
        </View>
        ...
        <View  style={{backgroundColor: '#fff', width: '30%', height: 50, borderLeftWidth: 1, borderLeftColor: '#f8ae4e', elevation: 5, marginBottom: 10, flexDirection: 'row', justifyContent: 'space-around'}}>
          ...
          <TouchableOpacity 
          onPress={()=>{
            Alert.alert("","Are you sure you want to delete "+item.DepartmentName+"?",[
        ...
        <View style={{marginTop: 'auto',backgroundColor:'#fff', elevation: 10, borderTopRightRadius: 30, borderTopLeftRadius: 30}}>
          ...
          <Text style={{fontFamily: 'Karla-Bold', fontSize: 18, alignSelf: 'center', color: '#000', paddingTop: 10}}>Edit Department</Text>
        ...
        onChangeText={text => setEditDepartment(text)}
        />
        </View>
